			...
			{	// in case the operand is a variable
				...
				{ // in the case of a variable on the stack or global
					...
					{	// in case there is the dereference accessor ('*' in C)
						...
						{
							throw NotImplementedException("cLoadOperand(): array access");
						}
...
public:
		...
		const CodeStorage::Fnc& entry)
	{
		...
		// check for garbage
		//append(new FI_check(nullptr, true));
		...
		assert(!assembly_->code_.empty());
		for (auto i = assembly_->code_.begin(); i != assembly_->code_.end()-1; ++i)
